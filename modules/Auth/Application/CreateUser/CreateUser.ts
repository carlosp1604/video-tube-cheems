...
export class CreateUser {
  ...
  public async create (request: CreateUserApplicationRequestInterface): Promise<void> {
    const verificationToken = await this.checkTokenExistsAndValidate(request.email, request.token)

    await this.checkIfEmailOrUsernameAreBeingUsed(request.email, request.username)

    const user = await this.buildUser(request, verificationToken)
  ...
  private async buildUser (
    request: CreateUserApplicationRequestInterface,
    ...
    try {
      return new User(
        ...
        nowDate,
        null,
        Relationship.createRelation(verificationToken),
